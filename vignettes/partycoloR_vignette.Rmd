...
vignette: >
  %\VignetteIndexEntry{Getting Started with partycoloR}
...
Note that the file extension should match the actual image format. Many Wikipedia
logos are SVGs, so check the URL to determine the correct extension.
...
If you don't have Wikipedia URLs, you can look up parties by name using the
[Party Facts](https://partyfacts.herokuapp.com/) database integration.

### Download Party Facts Data

```{r partyfacts-download}
...
Starting with version 0.3.0, `partycoloR` includes pre-scraped color and logo
data for major political parties. This means faster lookups without network
requests:
...
The bundled data covers parties from G20 countries and major European democracies.
If a party is not in the bundled data, functions automatically fall back to live
Wikipedia scraping.
...
2. **Cache Party Facts data**: If you'll be doing multiple lookups, call
   `get_partyfacts_wikipedia()` once and pass the result to `lookup_party_url()`.

3. **Handle missing data**: Always check for NA values in the results, as some
   Wikipedia pages may not have color information or may have unexpected
   formatting.

4. **Wikipedia changes**: The scraping depends on Wikipedia's HTML structure.
   If results seem wrong, the Wikipedia page format may have changed.
...
- [ParlGov](https://www.parlgov.org/) - Database on parties, elections and cabinets from EU and OECD democracies
- [ParlGov Dashboard](https://lukas-warode.shinyapps.io/ParlGov_Dashboard/) - R Shiny dashboard with data from ParlGov and party colors from `partycoloR`
